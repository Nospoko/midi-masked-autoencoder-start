...
class MidiDataset(Dataset):
    def __init__(
        ...
        self.mean_start = time_normalization_features["mean_start"]
        self.std_start = time_normalization_features["std_start"]
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        data = {
            ...
            # "sequence_duration": record["sequence_duration"],
            "pitch": torch.tensor(record["pitch"], dtype=torch.long) - 21,
            "velocity": (torch.tensor(record["velocity"], dtype=torch.float) / 64) - 1,
            "start": (torch.tensor(record["start"], dtype=torch.float) - self.mean_start) / self.std_start,
