# taken from: https://github.com/lucidrains/gradnorm-pytorch
...
from beartype.typing import Dict, List, Tuple, Union, Optional, NamedTuple
...
class GradNormLossWeighter(Module):
    ...
    def forward(
        self,
        losses: Union[Dict[str, Tensor], List[Tensor], Tuple[Tensor], Tensor],
        activations: Optional[
            Tensor
        ] = None,  # in the paper, they used the grad norm of penultimate parameters from a backbone layer. but this could also be activations (say shared image being fed to multiple discriminators)
        ...
        **backward_kwargs,
        ...
        if isinstance(losses, dict):
            ...
            assert input_loss_names == set(
                self.loss_names
            ), f"expect losses named {self.loss_names} but received {input_loss_names}"
        ...
        assert (
            losses.numel() == self.num_losses
        ), f"you instantiated with {self.num_losses} losses but passed in {losses.numel()} losses"

        total_weighted_loss = (losses * self.loss_weights.detach()).sum()

        (total_weighted_loss * scale).backward(**{**backward_kwargs, "retain_graph": not freeze})
        ...
        if self.frozen or freeze or not self.training or step < self.update_after_step or (step % self.update_every) != 0:
        ...
        if self.has_restoring_force:
            if not self.initted.item():
                ...
                self.initial_losses.lerp_(meaned_losses, 1.0 - self.initial_losses_decay)
